...
namespace GNC {
    ...
    class NodeAPI : public rclcpp::Node {
        public:
            NodeAPI(const std::string& node_name="gnc_node") : Node(node_name) {
                init_publishers_subscribers();

                proccess_timer_ = this->create_wall_timer(
        ...
        protected:
            ...
            Task wait4connect();
            Task wait4start();
            Task initialize_local_frame();
            Task arm();
            Task takeoff(float takeoff_alt);
            int check_waypoint_reached(float pos_tolerance=0.3, float heading_tolerance=5.0);
            Task go_to_waypoint(const WayPoint waypoint, float pos_tolerance=0.3, float heading_tolerance=5.0);
            Task set_mode(std::string mode);
            Task land();
            Task set_speed(float speed__mps);
            Task auto_set_current_waypoint(int seq);
            Task set_yaw(float angle, float speed, float dir, float absolute_rel);
            Task takeoff_global(float lat, float lon, float alt);
            ...
            std::queue<Task> mission_queue;
            void processQueue();
            rclcpp::TimerBase::SharedPtr proccess_timer_;
    };
};
